...
import { StrainCopyDialog } from "../strain-copy-dialog";
...
describe("StrainCopyDialog", () => {
  it.each(choices)("maps %s to the copy callback", async (label, mode, headers) => {
  ...
  it("keeps the dialog open and displays the original copy error", async () => {
    ...
    render(
      <StrainCopyDialog
        ...
        onOpenChange={onOpenChange}
        selectedCount={1}
        onCopy={vi.fn().mockRejectedValue(new Error("Clipboard permission denied"))}
      />,
    );
