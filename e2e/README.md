...
- **`taxonomyTreeScenarioOverrides`** — the one named bundle scoped to a *boundary* rather than a resource. The Taxa Tree reads from its own same-origin route (`/api/taxonomy-tree/children` and `/api/taxonomy-tree/child-counts`), not from `/api/data/taxonomy`, so `taxonomyScenarioOverrides` does not cover it. The bundle is data-free (`{rows: []}` / `{counts: {}}`); a spec that needs real tree nodes prepends its own content-bearing entries, which win under first-match ordering (see `e2e/tests/taxonomy-tree.spec.ts`). Any spec that mounts the tree needs it — the strict guard aborts an unmocked `/api/**` request and fails the test on teardown.
