...
- Collections whose row selection resolves to a set of Genome IDs (Strain via `genome_ids`, Genome via `genome_id`) share `CollectionSelectionActions` (`src/components/views/collection-selection-actions.tsx`) for the COPY ROWS, SERVICES, GENOMES, and GROUP sidebar actions, along with `CollectionCopyDialog` and `SelectionServiceChooser`. Row ceilings and ID extraction live in `src/lib/views/collection-selection.ts`. `ResourceCollection` picks the resource's action set from `selectionActionsConfigByResource` and keeps dispatching the actions the component does not own (e.g. GENOME) through `onOtherAction`. Multi-consumer pieces stay resource-neutral in name; single-consumer ones (such as `TaxonomyServiceChooser`) keep their resource prefix.
